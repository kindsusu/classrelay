...
**The window work is idempotent.** The backend answers every five-second heartbeat with a state frame, and a same-revision frame is accepted on purpose — that is what renews the lease the 15-second local watchdog depends on. The Agent therefore reaches `applyAgentWindow()` all class long, so it compares the mode it wants against what the window actually reports (`isVisible()`, `isAlwaysOnTop()`, `isKiosk()`) and applies only what differs. A repeated same-mode state performs no window calls at all, while a window that drifted — kiosk or always-on-top dropped by the OS, or minimised out of the class view — is repaired on the next state. Focus is taken only on a real mode transition: pulling focus every five seconds is exactly what `broadcast` must not do to a student who is deliberately left free to work in another window. Entering presentation mode is one `setKiosk(true)` call with no separate fullscreen call, because on Windows Electron implements kiosk as the fullscreen transition itself, so doing both transitioned the window twice in a single pass. Releasing to practice stays unconditional — hiding the window and dropping kiosk is the visible half of releasing a student, and it must never depend on a belief about the window's state. The renderer is still told the mode on every accepted state; only the window calls are conditional.
...
**No class mode prints the `Ctrl+Shift+F12` hint on a student screen.** The shortcut itself is unchanged — it still works in every mode and is still registered globally by the main process; only the on-screen advertisement is gone. `lecture` dropped the hint first; `lock`'s shield kept it on the theory that a shield covering the video makes the text harmless, but that reasoning was wrong — the risk was never obstruction, it was disclosure, since any student who can read the key combination can use it to break out of the lock. The shield now tells the student to look at the instructor in the room and to flag problems to the instructor, and it names no key. Because the shortcut is never shown to a student on any device or in any mode, briefing the on-site staff on it is a hard precondition, not a rollout nicety: enable `nativeInputLock` on a device only after the on-site staff for that room already know the shortcut, since no student-facing screen will ever teach it to them.
...
Press `Ctrl+Shift+F12` to release the classroom view immediately. No student-facing screen names this combination in any mode (see "What the student screen shows" above), so briefing the on-site staff on it is a hard precondition before `nativeInputLock` is enabled on any device — it must not be left to the shield's text to teach them. An Agent also releases fullscreen and input suppression if backend state is unavailable for 15 seconds, its WebRTC connection fails, the associated process exits, or its lease expires. A released command revision is not restored by reconnection.

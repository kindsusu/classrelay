...
**`lecture`는 더 이상 `Ctrl+Shift+F12` 안내를 화면에 띄우지 않습니다.** 단축키 자체는 메인 프로세스가 전역으로 등록하므로 모든 모드에서 동작하고, 화면의 안내만 사라졌습니다. 강사가 없애 달라고 한 것이 바로 그 상주 문구입니다. `lock` 차단막은 설계상 영상을 덮으므로 그 안의 안내는 아무것도 가리지 않아 그대로 남겼습니다. 도입 계획대로 수업 전에 현장 담당자에게 단축키를 반드시 사전 안내하세요 — `lecture`로 잠긴 학생은 자기 화면에서 단축키를 읽을 수 없습니다.

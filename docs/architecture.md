...
- **Every Controller button action is bounded.** `getDisplayMedia` cannot be cancelled and, on real hardware, once never returned — not even `GET /api/ice` went out and the instructor sat in front of an app whose buttons were all disabled. The capture call is now raced against a 10-second timeout (a stream that arrives after it is stopped on the spot), and the busy guard around every mode button releases the buttons after 30 seconds regardless. A capped action is assigned a stale generation: if it reaches `/api/mode` later it cancels itself, and if the server accepted the mode only after the cap, the Controller immediately returns the class to practice. A stale "connection closed" message is cleared when the socket reconnects; other errors stay visible.
- **One process per device.** The apps take Electron's single-instance lock at startup; a second launch of the same folder quits before creating a window, socket, or IPC. Two processes with one device identity would otherwise supersede each other on the server (`1012`) forever. For the instructor a second launch just raises the existing window; the student app ignores it.
...
On the student side the quit frame runs the normal teardown (input guard unlocked, kiosk left, control socket closed) and then `app.quit()`. Because a graceful Electron quit can stall in window teardown — observed once on real hardware as a student process that had stopped streaming but was still running — a forced `app.exit(0)` follows three seconds later. The timer is unreferenced, so a shutdown that completes normally never waits for it.

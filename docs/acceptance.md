...
Automated tests do not replace validation of actual streaming, WebSocket recovery, and Windows input locking. The following checks must be completed before operation; their default status is not run.
...
| 1 device | Full-screen broadcast → practice | Connected student releases immediately after the pushed practice command |
...
| 10 devices | Repeated broadcast/practice for one hour | Immediate command delivery and stable recovery without stream or process growth |
...
Perform these checks on company-owned training devices after showing the on-site operator how to use emergency release. Before deploying to 30 devices, validate forced shutdown and network loss on at least one device. No live deployment or hardware validation has been performed for this PoC yet.

...
- **강사 앱의 모든 버튼 동작에는 상한 시간이 있다.** `getDisplayMedia`는 취소할 수 없고, 실기기에서 한 번 끝내 돌아오지 않았다 — `GET /api/ice`조차 나가지 않았고 강사는 버튼이 전부 비활성화된 앱 앞에 앉아 있었다. 이제 캡처 호출은 10초 상한과 경쟁시키고(그 뒤 도착한 스트림은 그 자리에서 멈춘다), 모드 버튼을 감싸는 busy 가드는 무슨 일이 있어도 30초 뒤 버튼을 푼다. 상한에 걸린 동작은 낡은 세대 번호를 받아 나중에 `/api/mode`에 이르면 스스로 취소하고, 서버가 상한 뒤에야 모드를 받아들였다면 강사 앱이 즉시 수업을 실습으로 되돌린다. 재접속 시 낡은 "연결 종료" 문구는 지우되 다른 오류 문구는 남긴다.
- **장비당 프로세스 하나.** 앱은 시작 시 Electron 단일 인스턴스 잠금을 잡는다. 같은 폴더를 두 번 실행하면 두 번째는 창·소켓·IPC를 만들기 전에 종료한다. 같은 장비 식별로 두 프로세스가 붙으면 서버가 서로를 `1012`로 끝없이 밀어낸다. 강사 앱은 두 번째 실행에 기존 창을 앞으로 가져올 뿐이고, 학생 앱은 무시한다.
...
학생 쪽에서 quit 프레임은 평소의 정리(입력 guard 해제, kiosk 해제, 제어 소켓 종료)를 거친 뒤 `app.quit()`을 부른다. Electron의 정중한 종료는 창 정리에서 멈출 수 있고 — 실기기에서 송출은 끊겼는데 프로세스는 살아 있는 학생 앱으로 한 번 관측됐다 — 그래서 3초 뒤 `app.exit(0)` 강제 종료가 따른다. 그 타이머는 unref 상태라 정상적으로 끝나는 종료는 이를 기다리지 않는다.
